...
import { HanzoPromptCache } from "./prompts/promptCache";
import { ChatPromptClient, TextPromptClient, type HanzoPromptClient } from "./prompts/promptClients";
import { getCommonReleaseEnvs } from "./release-env";
import {
  HanzoPersistedProperty,
  type ChatMessage,
  type CreateChatPromptBody,
  type CreateHanzoDatasetBody,
  type CreateHanzoDatasetItemBody,
  type CreateHanzoDatasetItemResponse,
  type CreateHanzoDatasetResponse,
  type CreateHanzoDatasetRunItemBody,
  type CreateHanzoDatasetRunItemResponse,
  type CreateHanzoEventBody,
  type CreateHanzoGenerationBody,
  type CreateHanzoPromptBody,
  type CreateHanzoPromptResponse,
  type GetMediaUploadUrlRequest,
  type GetMediaUploadUrlResponse,
  type PatchMediaBody,
  type CreateHanzoScoreBody,
  type CreateHanzoSpanBody,
  type CreateHanzoTraceBody,
  type CreatePromptBody,
  type CreateTextPromptBody,
  type DatasetItem,
  type DeferRuntime,
  type EventBody,
  type GetHanzoDatasetItemsQuery,
  type GetHanzoDatasetItemsResponse,
  type GetHanzoDatasetParams,
  type GetHanzoDatasetResponse,
  type GetHanzoDatasetRunParams,
  type GetHanzoDatasetRunResponse,
  type GetHanzoDatasetRunsQuery,
  type GetHanzoDatasetRunsResponse,
  type GetHanzoObservationResponse,
  type GetHanzoObservationsQuery,
  type GetHanzoObservationsResponse,
  type GetHanzoPromptResponse,
  type GetHanzoSessionsQuery,
  type GetHanzoSessionsResponse,
  type GetHanzoTraceResponse,
  type GetHanzoTracesQuery,
  type GetHanzoTracesResponse,
  ...
  type UpdateHanzoGenerationBody,
  type UpdateHanzoSpanBody,
  type GetMediaResponse,
  type UpdatePromptBody,
} from "./types";
import { HanzoMedia, type HanzoMediaResolveMediaReferencesParams } from "./media/HanzoMedia";
import {
  currentISOTime,
  encodeQueryParams,
  generateUUID,
...
export * from "./media/HanzoMedia";

export { HanzoMemoryStorage } from "./storage-memory";
export type { HanzoPromptRecord } from "./types";
...
const MAX_BATCH_SIZE_BYTES = getEnv("LANGFUSE_MAX_BATCH_SIZE_BYTES")
  ...
  : 2_500_000;
const ENVIRONMENT_PATTERN = /^(?!hanzo)[a-z0-9_-]+$/;

class HanzoFetchHttpError extends Error {
  name = "HanzoFetchHttpError";
  body: string | undefined;

  constructor(
    public response: HanzoFetchResponse,
    body: string
  ) {
    super("HTTP error while fetching Hanzo: " + response.status + " and body: " + body);
  }
}

class HanzoFetchNetworkError extends Error {
  name = "HanzoFetchNetworkError";

  constructor(public error: unknown) {
    super("Network error while fetching Hanzo", error instanceof Error ? { cause: error } : {});
  }
}

function isHanzoFetchHttpError(error: any): error is HanzoFetchHttpError {
  return typeof error === "object" && error.name === "HanzoFetchHttpError";
}

function isHanzoFetchNetworkError(error: any): error is HanzoFetchNetworkError {
  return typeof error === "object" && error.name === "HanzoFetchNetworkError";
}

function isHanzoFetchError(err: any): boolean {
  return isHanzoFetchHttpError(err) || isHanzoFetchNetworkError(err);
}
...
function logIngestionError(error: any): void {
  if (isHanzoFetchHttpError(error)) {
    const code = error.response.status;
    const errorResponse = getErrorResponseByCode(code);
    console.error("[Hanzo SDK]", errorResponse, `Error details: ${error}`);
  } else if (isHanzoFetchNetworkError(error)) {
    console.error("[Hanzo SDK] Network error: ", error);
  } else {
    console.error("[Hanzo SDK] Unknown error:", error);
  }
}

abstract class HanzoCoreStateless {
  // options
  ...
  abstract fetch(url: string, options: HanzoFetchOptions): Promise<HanzoFetchResponse>;
  abstract getLibraryId(): string;
  abstract getLibraryVersion(): string;

  // This is our abstracted storage. Each implementation should handle its own
  abstract getPersistedProperty<T>(key: HanzoPersistedProperty): T | undefined;
  abstract setPersistedProperty<T>(key: HanzoPersistedProperty, value: T | null): void;

  constructor(params: HanzoCoreOptions) {
    const { publicKey, secretKey, enabled, _projectId, _isLocalEventExportEnabled, ...options } = params;

    this._events.on("error", (payload) => {
      console.error(`[Hanzo SDK] ${typeof payload === "string" ? payload : JSON.stringify(payload)}`);
    });

    this.enabled = enabled === false ? false : true;
    this.publicKey = publicKey ?? "";
    this.secretKey = secretKey;
    this.baseUrl = removeTrailingSlash(options?.baseUrl || "https://cloud.hanzo.ai");
    ...
    if (this.sampleRate) {
      this._events.emit("debug", `Hanzo trace sampling enabled with sampleRate ${this.sampleRate}.`);
    }
    ...
    if (this.environment && !ENVIRONMENT_PATTERN.test(this.environment)) {
      this._events.emit(
        "error",
        `Invalid tracing environment set: ${this.environment} . Environment must match regex ${ENVIRONMENT_PATTERN}. Events will be rejected by Hanzo server.`
      );
    }
  ...
  debug(enabled: boolean = true): void {
    ...
    if (enabled) {
      this.removeDebugCallback = this.on("*", (event, payload) => {
        ...
        console.log("[Hanzo Debug]", event, JSON.stringify(payload));
      });
    }
  }
   ...
   ***/
  protected traceStateless(body: CreateHanzoTraceBody): string {
    ...
    const parsedBody: CreateHanzoTraceBody = {
      id,
      release,
  ...
  protected eventStateless(body: CreateHanzoEventBody): string {
    const { id: bodyId, startTime: bodyStartTime, ...rest } = body;

    const id = bodyId ?? generateUUID();

    const parsedBody: CreateHanzoEventBody = {
      id,
  ...
  protected spanStateless(body: CreateHanzoSpanBody): string {
    const { id: bodyId, startTime: bodyStartTime, ...rest } = body;

    const id = bodyId || generateUUID();

    const parsedBody: CreateHanzoSpanBody = {
      id,
      startTime: bodyStartTime ?? new Date(),
      environment: this.environment,
      ...rest,
  ...
  protected generationStateless(
    body: Omit<CreateHanzoGenerationBody, "promptName" | "promptVersion"> & PromptInput
  ): string {
    ...
    const parsedBody: CreateHanzoGenerationBody = {
      id,
      startTime: bodyStartTime ?? new Date(),
      environment: this.environment,
      ...promptDetails,
  ...
  protected scoreStateless(body: CreateHanzoScoreBody): string {
    const { id: bodyId, ...rest } = body;

    const id = bodyId || generateUUID();

    const parsedBody: CreateHanzoScoreBody = {
      id,
      environment: this.environment,
      ...rest,
    };
  ...
  protected updateSpanStateless(body: UpdateHanzoSpanBody): string {
    this.enqueue("span-update", body);
    return body.id;
  }

  protected updateGenerationStateless(
    body: Omit<UpdateHanzoGenerationBody, "promptName" | "promptVersion"> & PromptInput
  ): string {
    const { prompt, ...rest } = body;
    const promptDetails =
      prompt && !prompt.isFallback ? { promptName: prompt.name, promptVersion: prompt.version } : {};

    const parsedBody: UpdateHanzoGenerationBody = {
      ...promptDetails,
      ...rest,
    };
  ...
  protected async _getDataset(name: GetHanzoDatasetParams["datasetName"]): Promise<GetHanzoDatasetResponse> {
  ...
  protected async _getDatasetItems(query: GetHanzoDatasetItemsQuery): Promise<GetHanzoDatasetItemsResponse> {
  ...
  async fetchTraces(query?: GetHanzoTracesQuery): Promise<GetHanzoTracesResponse> {
    ...
    const { data, meta } = await this.fetchAndLogErrors<GetHanzoTracesResponse>(
  ...
  async fetchTrace(traceId: string): Promise<{ data: GetHanzoTraceResponse }> {
    const res = await this.fetchAndLogErrors<GetHanzoTraceResponse>(
  ...
  async fetchObservations(query?: GetHanzoObservationsQuery): Promise<GetHanzoObservationsResponse> {
    // destructure the response into data and meta to be explicit about the shape of the response and add type-warnings in case the API changes
    const { data, meta } = await this.fetchAndLogErrors<GetHanzoObservationsResponse>(
  ...
  async fetchObservation(observationId: string): Promise<{ data: GetHanzoObservationResponse }> {
    const res = await this.fetchAndLogErrors<GetHanzoObservationResponse>(
  ...
  async fetchSessions(query?: GetHanzoSessionsQuery): Promise<GetHanzoSessionsResponse> {
    // destructure the response into data and meta to be explicit about the shape of the response and add type-warnings in case the API changes
    const { data, meta } = await this.fetchAndLogErrors<GetHanzoSessionsResponse>(
  ...
  async getDatasetRun(params: GetHanzoDatasetRunParams): Promise<GetHanzoDatasetRunResponse> {
  ...
  async getDatasetRuns(
    datasetName: string,
    query?: GetHanzoDatasetRunsQuery
  ): Promise<GetHanzoDatasetRunsResponse> {
    return this.fetchAndLogErrors(
  ...
  async createDatasetRunItem(body: CreateHanzoDatasetRunItemBody): Promise<CreateHanzoDatasetRunItemResponse> {
    return this.fetchAndLogErrors(
  ...
  ): Promise<CreateHanzoDatasetResponse> {
    const body: CreateHanzoDatasetBody = typeof dataset === "string" ? { name: dataset } : dataset;
    return this.fetchAndLogErrors(
   ...
   * @returns A promise that resolves to the response of the create operation.
   */
  async createDatasetItem(body: CreateHanzoDatasetItemBody): Promise<CreateHanzoDatasetItemResponse> {
    return this.fetchAndLogErrors(
  ...
  async getDatasetItem(id: string): Promise<CreateHanzoDatasetItemResponse> {
    return this.fetchAndLogErrors(
  ...
  async createPromptStateless(body: CreateHanzoPromptBody): Promise<CreateHanzoPromptResponse> {
    return this.fetchAndLogErrors(
  ...
  ): Promise<GetHanzoPromptResponse> {
    const encodedName = encodeURIComponent(name);
    const params = new URLSearchParams();
        ...
        const res = await this.fetch(url, this._getFetchOptions({ method: "GET", fetchTimeout: requestTimeout })).catch(
          (e) => {
            if (e.name === "AbortError") {
              throw new HanzoFetchNetworkError("Fetch request timed out");
            }
            throw new HanzoFetchNetworkError(e);
          }
        );

        if (res.status >= 500) {
          throw new HanzoFetchHttpError(res, await res.text());
        }
   ...
   ***/
  protected enqueue(type: HanzoObject, body: EventBody): void {
    if (!this.enabled) {
      return;
    }
    ...
    if (!traceId) {
      this._events.emit(
        "warning",
        "Failed to parse traceID for sampling. Please open a Github issue in https://github.com/hanzo/hanzo/issues/new/choose"
      );
  ...
  protected async processEnqueueEvent(type: HanzoObject, body: EventBody): Promise<void> {
    ...
    const queue = this.getPersistedProperty<HanzoQueueItem[]>(HanzoPersistedProperty.Queue) || [];
    ...
    this.setPersistedProperty<HanzoQueueItem[]>(HanzoPersistedProperty.Queue, queue);
  ...
  protected async processMediaInEvent(type: HanzoObject, body: EventBody): Promise<void> {
    if (!body) {
      return;
    }
  ...
  protected parseTraceId(type: HanzoObject, body: EventBody): string | null | undefined {
  ...
  protected async findAndProcessMedia({
    ...
    const processRecursively = async (data: any, level: number): Promise<any> => {
      if (typeof data === "string" && data.startsWith("data:")) {
        const media = new HanzoMedia({ base64DataUri: data });
      ...
      if (data instanceof HanzoMedia || Object.prototype.toString.call(data) === "[object HanzoMedia]") {
  ...
  private async uploadMediaWithBackoff(params: {
    ...
    maxRetries: number;
    baseDelay: number;
  }): Promise<HanzoFetchResponse | undefined> {
  ...
  async flushAsync(): Promise<void> {
    ...
    return new Promise((resolve, _reject) => {
      try {
        ...
        console.error("[Hanzo SDK] Error while flushing Hanzo", e);
      }
    });
  }
  ...
  flush(callback?: (err?: any, data?: any) => void): void {
    ...
    this.setPersistedProperty<HanzoQueueItem[]>(HanzoPersistedProperty.Queue, [...remainingItems, ...queue]);
  ...
  public processQueueItems(
    queue: HanzoQueueItem[],
    MAX_MSG_SIZE: number,
    BATCH_SIZE_LIMIT: number
  ): { processedItems: HanzoQueueItem[]; remainingItems: HanzoQueueItem[] } {
    let totalSize = 0;
    const processedItems: HanzoQueueItem[] = [];
    const remainingItems: HanzoQueueItem[] = [];
  ...
  _getFetchOptions(p: {
    method: HanzoFetchOptions["method"];
    body?: HanzoFetchOptions["body"];
    fetchTimeout?: number;
  }): HanzoFetchOptions {
    const fetchOptions: HanzoFetchOptions = {
      method: p.method,
      headers: {
  ...
  private async fetchWithRetry(
    url: string,
    options: HanzoFetchOptions,
    retryOptions?: RetriableOptions
  ): Promise<HanzoFetchResponse> {
    ...
    return await retriable(
      async () => {
        let res: HanzoFetchResponse<IngestionReturnType> | null = null;
        try {
        ...
        if (res.status < 200 || res.status >= 400) {
          ...
          throw new HanzoFetchHttpError(res, JSON.stringify(body));
        }
        const returnBody = await res.json();
        if (res.status === 207 && returnBody.errors.length > 0) {
          throw new HanzoFetchHttpError(res, JSON.stringify(returnBody.errors));
        }
  ...
  private async fetchAndLogErrors<T>(url: string, options: HanzoFetchOptions): Promise<T> {
    const res = await this.fetch(url, options);
    const data = await res.json();

    if (res.status < 200 || res.status >= 400) {
      logIngestionError(new HanzoFetchHttpError(res, JSON.stringify(data)));
    }
  ...
  async shutdownAsync(): Promise<void> {
    ...
    try {
      ...
      console.error("[Hanzo SDK] Error while shutting down Hanzo", e);
    }
  }
...
export abstract class HanzoWebStateless extends HanzoCoreStateless {
  constructor(params: Omit<HanzoCoreOptions, "secretKey">) {
  ...
  async score(body: CreateHanzoScoreBody): Promise<this> {
...
export abstract class HanzoCore extends HanzoCoreStateless {
  private _promptCache: HanzoPromptCache;
  ...
  constructor(params: HanzoCoreOptions) {
    ...
    if (_isLocalEventExportEnabled) {
      ...
      console.warn("Hanzo is disabled. No observability data will be sent to Hanzo.");
    } else if (!secretKey) {
      isObservabilityEnabled = false;
      console.warn(
    ...
    this._promptCache = new HanzoPromptCache();
  }

  trace(body?: CreateHanzoTraceBody): HanzoTraceClient {
    const id = this.traceStateless(body ?? {});
    const t = new HanzoTraceClient(this, id);
    if (getEnv("DEFER") && body) {
      try {
        const deferRuntime = getEnv<DeferRuntime>("__deferRuntime");
        if (deferRuntime) {
          deferRuntime.hanzoTraces([
            {
              id: id,
              name: body.name || "",
              url: t.getTraceUrl(),
  ...
  generation(
    ...
    return new HanzoGenerationClient(this, id, traceId);
  }

  event(body: CreateHanzoEventBody): HanzoEventClient {
    const traceId = body.traceId || this.traceStateless({ name: body.name });
    const id = this.eventStateless({ ...body, traceId });
    return new HanzoEventClient(this, id, traceId);
  }

  score(body: CreateHanzoScoreBody): this {
    this.scoreStateless(body);
    return this;
  }
  ...
  async getDataset(
    ...
    const items: GetHanzoDatasetItemsResponse["data"] = [];
  ...
  async createPrompt(body: CreatePromptBody): Promise<HanzoPromptClient> {
  ...
  async updatePrompt(body: { name: string; version: number; newLabels: string[] }): Promise<HanzoPromptClient> {
  ...
  private async _fetchPromptAndUpdateCache(params: {
    ...
    try {
      ...
      console.error(`[Hanzo SDK] Error while fetching prompt '${cacheKey}':`, error);
   ...
   * in the format "@@@hanzoMedia:...@@@". When found, it fetches the actual media content using the provided
   * Hanzo client and replaces the reference string with a base64 data URI.
   *
   * If fetching media content fails for a reference string, a warning is logged and the reference string is left unchanged.
   *
   * @param params - Configuration object
   * @param params.obj - The object to process. Can be a primitive value, array, or nested object
   * @param params.hanzoClient - Hanzo client instance used to fetch media content
   ...
   * ```typescript
   * const obj = {
   *   image: "@@@hanzoMedia:type=image/jpeg|id=123|source=bytes@@@",
   *   nested: {
   *     pdf: "@@@hanzoMedia:type=application/pdf|id=456|source=bytes@@@"
   *   }
   * };
   *
   * const result = await HanzoMedia.resolveMediaReferences({
   *   obj,
   *   hanzoClient
   * });
   *
  ...
  public async resolveMediaReferences<T>(
    params: Omit<HanzoMediaResolveMediaReferencesParams<T>, "hanzoClient">
  ): Promise<T> {
    const { obj, ...rest } = params;

    return HanzoMedia.resolveMediaReferences<T>({ ...rest, hanzoClient: this, obj });
  }
  _updateSpan(body: UpdateHanzoSpanBody): this {
    this.updateSpanStateless(body);
    return this;
  }

  _updateGeneration(body: UpdateHanzoGenerationBody): this {
    this.updateGenerationStateless(body);
    return this;
  }
}

export abstract class HanzoObjectClient {
  public readonly client: HanzoCore;
  ...
  event(body: Omit<CreateHanzoEventBody, "traceId" | "parentObservationId">): HanzoEventClient {
  ...
  span(body: Omit<CreateHanzoSpanBody, "traceId" | "parentObservationId">): HanzoSpanClient {
  ...
  generation(
    body: Omit<CreateHanzoGenerationBody, "traceId" | "parentObservationId" | "promptName" | "promptVersion"> &
      PromptInput
  ): HanzoGenerationClient {
  ...
  score(body: Omit<CreateHanzoScoreBody, "traceId" | "parentObservationId">): this {
    this.client.score({
      ...body,
      traceId: this.traceId,
...
export class HanzoTraceClient extends HanzoObjectClient {
  constructor(client: HanzoCore, traceId: string) {
    super({ client, id: traceId, traceId, observationId: null });
  }

  update(body: Omit<CreateHanzoTraceBody, "id">): this {
    this.client.trace({
      ...body,
      id: this.id,
    });
...
abstract class HanzoObservationClient extends HanzoObjectClient {
  constructor(client: HanzoCore, id: string, traceId: string) {
    super({ client, id, traceId, observationId: id });
  }
}

export class HanzoSpanClient extends HanzoObservationClient {
  constructor(client: HanzoCore, id: string, traceId: string) {
    super(client, id, traceId);
  }

  update(body: Omit<UpdateHanzoSpanBody, "id" | "traceId">): this {
  ...
  end(body?: Omit<UpdateHanzoSpanBody, "id" | "endTime" | "traceId">): this {
    this.client._updateSpan({
      ...body,
      id: this.id,
      traceId: this.traceId,
...
export class HanzoGenerationClient extends HanzoObservationClient {
  constructor(client: HanzoCore, id: string, traceId: string) {
    super(client, id, traceId);
  }

  update(
    body: Omit<UpdateHanzoGenerationBody, "id" | "traceId" | "promptName" | "promptVersion"> & PromptInput
  ): this {
    ...
    body?: Omit<UpdateHanzoGenerationBody, "id" | "traceId" | "endTime" | "promptName" | "promptVersion"> &
      PromptInput
  ): this {
    this.client._updateGeneration({
      ...body,
...
export class HanzoEventClient extends HanzoObservationClient {
  constructor(client: HanzoCore, id: string, traceId: string) {
    super(client, id, traceId);
  }
}
