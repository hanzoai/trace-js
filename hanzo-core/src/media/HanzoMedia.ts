import { type HanzoCore } from "../index";
...
export type HanzoMediaResolveMediaReferencesParams<T> = {
  obj: T;
...
/**
 * A class for wrapping media objects for upload to Hanzo.
 *
 * This class handles the preparation and formatting of media content for Hanzo,
...
class HanzoMedia {
  ...
  constructor(params: {
    ...
    if (base64DataUri) {
      ...
      console.error("base64DataUri, or contentBytes and contentType, or filePath must be provided to HanzoMedia");
    }
  }
  ...
  toJSON(): string | undefined {
    if (!this._contentType || !this._source || !this._mediaId) {
      return `<Upload handling failed for HanzoMedia of type ${this._contentType}>`;
    }

    return `@@@hanzoMedia:type=${this._contentType}|id=${this._mediaId}|source=${this._source}@@@`;
  }
   ...
   *     "@@@hanzoMedia:type=image/jpeg|id=some-uuid|source=base64DataUri@@@"
   *
  ...
  public static parseReferenceString(referenceString: string): ParsedMediaReference {
    const prefix = "@@@hanzoMedia:";
    const suffix = "@@@";

    if (!referenceString.startsWith(prefix)) {
      throw new Error("Reference string does not start with '@@@hanzoMedia:type='");
    }
   ...
   * ```
   */
  public static async resolveMediaReferences<T>(params: HanzoMediaResolveMediaReferencesParams<T>): Promise<T> {
    const { obj, hanzoClient, maxDepth = 10 } = params;
      ...
      if (typeof obj === "string") {
        ...
        await Promise.all(
          referenceStringMatches.map(async (referenceString) => {
            try {
              const parsedMediaReference = HanzoMedia.parseReferenceString(referenceString);
              const mediaData = await hanzoClient.fetchMedia(parsedMediaReference.mediaId);
              const mediaContent = await hanzoClient.fetch(mediaData.url, { method: "GET", headers: {} });
...
export { HanzoMedia, type MediaContentType, type ParsedMediaReference };
