import type { HanzoPromptClient } from "./promptClients";

export const DEFAULT_PROMPT_CACHE_TTL_SECONDS = 60;

class HanzoPromptCacheItem {
  private _expiry: number;

  constructor(
    public value: HanzoPromptClient,
    ttlSeconds: number
  ) {
...
export class HanzoPromptCache {
  private _cache: Map<string, HanzoPromptCacheItem>;
  private _defaultTtlSeconds: number;
  private _refreshingKeys: Map<string, Promise<void>>;

  constructor() {
    this._cache = new Map<string, HanzoPromptCacheItem>();
    this._defaultTtlSeconds = DEFAULT_PROMPT_CACHE_TTL_SECONDS;
    this._refreshingKeys = new Map<string, Promise<void>>();
  }

  public getIncludingExpired(key: string): HanzoPromptCacheItem | null {
    return this._cache.get(key) ?? null;
  }

  public set(key: string, value: HanzoPromptClient, ttlSeconds?: number): void {
    const effectiveTtlSeconds = ttlSeconds ?? this._defaultTtlSeconds;
    this._cache.set(key, new HanzoPromptCacheItem(value, effectiveTtlSeconds));
  }
