import { HanzoMedia, type MediaContentType } from "../src/media/HanzoMedia";

describe("HanzoMedia", () => {
  describe("constructor", () => {
    it("should handle base64 data URI input", () => {
      const base64DataUri = "data:image/jpeg;base64,/9j/4AAQSkZJRg==";
      const media = new HanzoMedia({ base64DataUri });
    ...
    it("should handle content bytes and type input", () => {
      ...
      const media = new HanzoMedia({ contentBytes, contentType });
    ...
    it("should handle file path input", () => {
      ...
      const media = new HanzoMedia({ filePath, contentType });
  ...
  describe("parseReferenceString", () => {
    it("should correctly parse valid reference strings", () => {
      const refString = "@@@hanzoMedia:type=application/pdf|id=e64a339d-5051-4ffa-8992-f8baa159720a|source=bytes@@@";
      const parsed = HanzoMedia.parseReferenceString(refString);
    ...
    it("should throw error for invalid reference strings", () => {
      ...
      expect(() => HanzoMedia.parseReferenceString(invalidRef)).toThrow();
    });
  });
  ...
  describe("toJSON", () => {
    it("should return formatted string when all required fields are present", () => {
      ...
      expect(JSON.parse(json)).toBe("@@@hanzoMedia:type=image/png|id=test-id|source=bytes@@@");
    });
