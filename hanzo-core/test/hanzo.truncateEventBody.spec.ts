import { createTestClient, type HanzoCoreTestClient } from "./test-utils/HanzoCoreTestClient";

describe("truncateEventBody", () => {
  let hanzo: HanzoCoreTestClient;

  beforeEach(() => {
    [hanzo] = createTestClient({
      publicKey: "pk-lf-111",
      secretKey: "sk-lf-111",
      flushAt: 1,
    });
  ...
  it("should return the original body if size is within limit", () => {
    ...
    const result = (hanzo as any).truncateEventBody(body, 1000);
  ...
  it("should truncate fields if body size exceeds limit", () => {
    ...
    const result = (hanzo as any).truncateEventBody(body, 500);
  ...
  it("should handle non-object bodies", () => {
    ...
    const result = (hanzo as any).truncateEventBody(body as any, 1000);
...
describe("getByteSize", () => {
  ...
  it("should correctly calculate byte size of strings", () => {
    expect((hanzo as any).getByteSize("hello")).toBe(7);
    expect((hanzo as any).getByteSize("こんにちは")).toBe(17); // 3 bytes per character
  });

  it("should correctly calculate byte size of objects", () => {
    const obj = { key: "value" };
    expect((hanzo as any).getByteSize(obj)).toBe(15); // {"key":"value"}
  });

  it("should correctly calculate byte size of arrays", () => {
    const arr = [1, 2, 3];
    expect((hanzo as any).getByteSize(arr)).toBe(7); // [1,2,3]
  });

  it("should handle empty objects and arrays", () => {
    expect((hanzo as any).getByteSize({})).toBe(2);
    expect((hanzo as any).getByteSize([])).toBe(2);
  });

  it("should handle null and undefined", () => {
    expect((hanzo as any).getByteSize(null)).toBe(4);
    expect((hanzo as any).getByteSize(undefined)).toBe(0);
  });
});
