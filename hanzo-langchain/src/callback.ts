import { Hanzo, type HanzoOptions } from "hanzo";
...
import type { ChatPromptClient, HanzoSpanClient, HanzoTraceClient, TextPromptClient } from "hanzo-core";
...
type RootParams = {
  root: HanzoTraceClient | HanzoSpanClient;
};
...
export class CallbackHandler extends BaseCallbackHandler {
  ...
  constructor(params?: ConstructorParams) {
    ...
    if (params && "root" in params) {
      this.hanzo = params.root.client as Hanzo;
  ...
  async flushAsync(): Promise<any> {
    return this.hanzo.flushAsync();
  }

  async shutdownAsync(): Promise<any> {
    return this.hanzo.shutdownAsync();
  }

  debug(enabled: boolean = true): void {
    this.hanzo.debug(enabled);
  ...
  async handleChainStart(
    ...
    try {
      ...
      this.registerHanzoPrompt(parentRunId, metadata);
      ...
      if (!parentRunId) {
        this.traceUpdates.set(runId, {
          tags,
          userId:
            metadata && "hanzoUserId" in metadata && typeof metadata["hanzoUserId"] === "string"
              ? metadata["hanzoUserId"]
              : undefined,
          sessionId:
            metadata && "hanzoSessionId" in metadata && typeof metadata["hanzoSessionId"] === "string"
              ? metadata["hanzoSessionId"]
              : undefined,
        });
      }
    } catch (e) {
  ...
  private registerHanzoPrompt(parentRunId?: string, metadata?: Record<string, unknown>): void {
    /*
    ...
    */
    if (metadata && "hanzoPrompt" in metadata && parentRunId) {
      this.promptToParentRunMap.set(parentRunId, metadata.hanzoPrompt as TextPromptClient | ChatPromptClient);
    }
  }

  private deregisterHanzoPrompt(runId: string): void {
  ...
  generateTrace(
    ...
    if (this.rootProvided && this.updateRoot) {
      if (this.rootObservationId) {
        this.hanzo._updateSpan({ id: this.rootObservationId, traceId: this.traceId, ...params });
      } else {
        this.hanzo.trace({ id: this.traceId, ...params });
      }
    }
  ...
  async handleGenerationStart(
    ...
    if (registeredPrompt && parentRunId) {
      this.deregisterHanzoPrompt(parentRunId);
    }

    this.hanzo.generation({
      id: runId,
      traceId: this.traceId,
  ...
  async handleChainEnd(outputs: ChainValues, runId: string, parentRunId?: string | undefined): Promise<void> {
    try {
      ...
      this.deregisterHanzoPrompt(runId);
    } catch (e) {
      this._log(e);
    }
  }
  ...
  updateTrace(runId: string, parentRunId: string | undefined, output: any): void {
    ...
    if (!parentRunId && this.traceId && this.traceId === runId) {
      this.hanzo.trace({ id: this.traceId, output: output, ...traceUpdates });
    }

    if (!parentRunId && this.traceId && this.rootProvided && this.updateRoot) {
      if (this.rootObservationId) {
        this.hanzo._updateSpan({ id: this.rootObservationId, traceId: this.traceId, output });
      } else {
        this.hanzo.trace({ id: this.traceId, output, ...traceUpdates });
      }
    }
  }
  ...
  joinTagsAndMetaData(
    ...
    return this.stripHanzoKeysFromMetadata(finalDict);
  }

  private stripHanzoKeysFromMetadata(metadata?: Record<string, unknown>): Record<string, unknown> | undefined {
    if (!metadata) {
      return;
    }

    const hanzoKeys = ["hanzoPrompt", "hanzoUserId", "hanzoSessionId"];

    return Object.fromEntries(Object.entries(metadata).filter(([key, _]) => !hanzoKeys.includes(key)));
  }
}
