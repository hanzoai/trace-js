...
import { Hanzo, type HanzoOptions, type HanzoPromptRecord } from "hanzo";

import type { ExportResult, ExportResultCode } from "@opentelemetry/core";

type HanzoExporterParams = {
  publicKey?: string;
  secretKey?: string;
  baseUrl?: string;
  debug?: boolean;
...
export class HanzoExporter implements SpanExporter {
  static hanzo: Hanzo | null = null; // Singleton instance
  private readonly debug: boolean;
  private readonly hanzo: Hanzo;

  constructor(params: HanzoExporterParams = {}) {
    this.debug = params.debug ?? false;

    if (!HanzoExporter.hanzo) {
      HanzoExporter.hanzo = new Hanzo({
        ...params,
        persistence: "memory",
      ...
      if (this.debug) {
        HanzoExporter.hanzo.debug();
      }
    }

    this.hanzo = HanzoExporter.hanzo; // store reference to singleton instance
  }
  ...
  private processTraceSpans(traceId: string, spans: ReadableSpan[]): void {
    ...
    const hanzoPrompt = this.parseHanzoPromptTraceAttribute(spans);
    const updateParent = this.parseHanzoUpdateParentTraceAttribute(spans);
    ...
    this.hanzo.trace(finalTraceParams);

    for (const span of spans) {
      if (this.isGenerationSpan(span)) {
        this.processSpanAsHanzoGeneration(finalTraceId, span, this.isRootAiSdkSpan(span, spans), hanzoPrompt);
      } else {
        this.processSpanAsHanzoSpan(
          finalTraceId,
          span,
  ...
  private processSpanAsHanzoSpan(
    traceId: string,
    span: ReadableSpan,
    isRootSpan: boolean,
    rootSpanName?: string
  ...
  private processSpanAsHanzoGeneration(
    traceId: string,
    span: ReadableSpan,
    isRootSpan: boolean,
    hanzoPrompt: HanzoPromptRecord | undefined
  ): void {
    const spanContext = span.spanContext();
    const attributes = span.attributes;
  ...
  private logDebug(message: string, ...args: any[]): void {
    ...
    console.log(`[${new Date().toISOString()}] [HanzoExporter] ${message}`, ...args);
  }
  ...
  async forceFlush(): Promise<void> {
    this.logDebug("Force flushing Hanzo...");

    await this.hanzo.flushAsync();
  }

  async shutdown(): Promise<void> {
    this.logDebug("Shutting down Hanzo...");

    await this.hanzo.shutdownAsync();
  }
  ...
  private parseTraceId(spans: ReadableSpan[]): string | undefined {
    return spans
      .map((span) => this.parseSpanMetadata(span)["hanzoTraceId"])
      .find((id) => Boolean(id))
      ?.toString();
  }
  ...
  private parseHanzoPromptTraceAttribute(spans: ReadableSpan[]): HanzoPromptRecord | undefined {
    const jsonPrompt = spans
      .map((span) => this.parseSpanMetadata(span)["hanzoPrompt"])
    ...
    try {
      if (jsonPrompt) {
        ...
        if (
          ...
          throw Error("Invalid hanzoPrompt");
        }
  ...
  private parseHanzoUpdateParentTraceAttribute(spans: ReadableSpan[]): boolean {
    return Boolean(
      spans.map((span) => this.parseSpanMetadata(span)["hanzoUpdateParent"]).find((val) => val != null) ?? true // default to true if no attribute is set
    );
  }
  ...
  private filterTraceAttributes(obj: Record<string, any>): Record<string, any> {
    ...
    return Object.entries(obj).reduce(
      (acc, [key, value]) => {
        if (!hanzoTraceAttributes.includes(key)) {
          acc[key] = value;
        }
