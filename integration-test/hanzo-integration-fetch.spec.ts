...
import Hanzo, { HanzoMedia } from "../hanzo";
...
describe("Hanzo (fetch)", () => {
  ...
  describe("core methods", () => {
    ...
    it("silently skips creating observations if Hanzo is disabled", async () => {
      const consoleSpy = jest.spyOn(console, "warn").mockImplementation(() => {});
      const hanzo = new Hanzo({ enabled: false });
      const fetchSpy = jest.spyOn(hanzo, "fetch");

      expect(consoleSpy).toHaveBeenCalledWith("Hanzo is disabled. No observability data will be sent to Hanzo.");
    ...
    it("create prompt", async () => {
      ...
      const createdPrompt = await hanzo.createPrompt({
        name: promptName,
        isActive: true,
      ...
      const fetchedPrompt = await hanzo.getPrompt(promptName);
    ...
    it("should fetch prompt by version and label", async () => {
      ...
      const defaultFetchedPrompt = await hanzo.getPrompt(promptName);
      ...
      const fetchedPrompt1 = await hanzo.getPrompt(promptName, 1);
      ...
      const fetchedPrompt2 = await hanzo.getPrompt(promptName, undefined, { label: "dev" });
      ...
      const fetchedPrompt3 = await hanzo.getPrompt(promptName, 2);
    ...
    it("create chat prompt", async () => {
      ...
      const fetchedPrompt = await hanzo.getPrompt(promptName, undefined, { type: "chat" });
    ...
    it("should not return fallback if fetch succeeds", async () => {
      ...
      const fetchedPrompt = await hanzo.getPrompt(promptName, undefined, {
    ...
    it("should return the fallback text prompt if cache empty and fetch fails", async () => {
      ...
      await expect(hanzo.getPrompt(promptName)).rejects.toThrow();

      const prompt = await hanzo.getPrompt(promptName, undefined, {
        fallback,
      });
    ...
    it("sampleRate can be passed as constructor arg", () => {
      ...
      const hanzo = new Hanzo({ sampleRate });

      expect((hanzo as any).sampleRate).toBe(sampleRate);
    });
    ...
    it("sampleRate can be passed as environment variable", () => {
      ...
      expect((hanzo as any).sampleRate).toBe(0.5);
    ...
    it("should sample trace Ids correctly", async () => {
      ...
      const hanzo = new Hanzo({ sampleRate: 0.5 });
      hanzo.debug();

      const inSampleTrace = hanzo.trace({ id: traceIdInSample, name: traceIdInSample });
      inSampleTrace.span({ name: "span" });
      inSampleTrace.generation({ name: "generation" });

      const outSampleTrace = hanzo.trace({ id: traceIdOutSample, name: traceIdOutSample });
    ...
    it("should mask data in the event body", async () => {
      ...
      const hanzo = new Hanzo({ mask });
      const traceId = randomUUID();

      const trace = hanzo.trace({ id: traceId, input: "confidential data" });
  ...
  it("should set environment if specified in constructor arg", async () => {
    ...
    const hanzo = new Hanzo({ environment });
    const traceId = randomUUID();

    const trace = hanzo.trace({ id: traceId });
  ...
  it("replace media reference string in object", async () => {
    ...
    const res = await hanzo.fetchTrace(trace.id);
    ...
    expect(res.data).toMatchObject({
      ...
      metadata: {
        context: {
          nested: expect.stringMatching(/^@@@hanzoMedia:type=audio\/wav\|id=.+\|source=base64_data_uri@@@$/),
        },
      },
    });

    const mediaReplacedTrace = await hanzo.resolveMediaReferences({
