...
import { CallbackHandler, Hanzo, type LlmMessage } from "../hanzo-langchain";
...
describe("Langchain", () => {
  ...
  //     const options = callback.hanzo._getFetchOptions({ method: "POST", body: "test" });
  //     expect(callback.hanzo.baseUrl).toEqual(LANGFUSE_BASEURL);
  //     expect(options).toMatchObject({
  //       headers: {
  //         "Content-Type": "application/json",
  //         "X-Hanzo-Sdk-Name": "trace-js",
  //         "X-Hanzo-Sdk-Variant": "hanzo",
  //         "X-Hanzo-Public-Key": LANGFUSE_PUBLIC_KEY,
  //         ...getHeaders(),
  //       },
  //       body: "test",
  //     });
  ...
  //     expect(callback.hanzo.baseUrl).toEqual("http://example.com");
  //     expect(options).toMatchObject({
  //       headers: {
  //         "Content-Type": "application/json",
  //         "X-Hanzo-Sdk-Name": "trace-js",
  ...
  describe("chains", () => {
    ...
    it("should execute tool call", async () => {
      ...
      const initialTrace = hanzo.trace({ name: "wikipedia-test" });
    ...
    it("create trace for callback", async () => {
      ...
      const trace = hanzo.trace({ name: "test-123" });
    ...
    it("create span for callback with span update", async () => {
      ...
      const trace = hanzo.trace({ name: "test-trace" });
    ...
    it("links a hanzo chat prompt to a langchain run", async () => {
      const hanzo = new Hanzo();
      ...
      // Fetch prompts
      const hanzoJokePrompt = await hanzo.getPrompt(jokePromptName, undefined, { type: "chat" });

      const langchainJokePrompt = ChatPromptTemplate.fromMessages(hanzoJokePrompt.getLangchainPrompt()).withConfig({
        metadata: { hanzoPrompt: hanzoJokePrompt },
      });
      ...
      expect((generation as any)["promptVersion"]).toBe(hanzoJokePrompt.version);
    });

    it("links a hanzo prompt to a langchain run", async () => {
      const hanzo = new Hanzo();
      ...
      // Fetch prompts
      const [hanzoJokePrompt, hanzoExplainPrompt] = await Promise.all([
        hanzo.getPrompt(jokePromptName),
        hanzo.getPrompt(explainPromptName),
      ]);

      const langchainJokePrompt = PromptTemplate.fromTemplate(hanzoJokePrompt.getLangchainPrompt()).withConfig({
        metadata: { hanzoPrompt: hanzoJokePrompt },
      });
      const langchainExplainPrompt = PromptTemplate.fromTemplate(hanzoExplainPrompt.getLangchainPrompt()).withConfig(
        {
          metadata: { hanzoPrompt: hanzoExplainPrompt },
        }
      );
      ...
      await chain.invoke(
        { topic: "vacation" },
        {
          ...
          runName: traceName,
          tags: ["langchain-tag"],
          metadata: { hanzoUserId: userId, hanzoSessionId: sessionId },
        }
      );
      await handler.shutdownAsync();
      ...
      expect((jokeGeneration as any)["promptName"]).toBe(hanzoJokePrompt.name);
      expect((jokeGeneration as any)["promptVersion"]).toBe(hanzoJokePrompt.version);

      expect((explainGeneration as any)["promptName"]).toBe(hanzoExplainPrompt.name);
      expect((explainGeneration as any)["promptVersion"]).toBe(hanzoExplainPrompt.version);
    });
    ...
    it("should export events in admin mode", async () => {
      ...
      await handler.flushAsync();
      const shutdownResult = await handler.hanzo._exportLocalEvents(projectId);
      if (!shutdownResult) {
