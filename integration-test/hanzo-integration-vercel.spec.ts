...
import { HanzoExporter } from "../hanzo-vercel";
...
describe("hanzo-integration-vercel", () => {
  let sdk: NodeSDK;

  beforeEach(() => {
    sdk = new NodeSDK({
      traceExporter: new HanzoExporter({ debug: false }),
  ...
  it("should trace a streamText call with linked prompts", async () => {
    ...
    const stream = await streamText({
      ...
      experimental_telemetry: {
        ...
        metadata: {
          hanzoTraceId: traceId,
          hanzoPrompt: fetchedPrompt.toJSON(),
          userId,
          sessionId,
          tags,
          ...metadata,
  ...
  it("should nest multiple generateText call under a trace", async () => {
    ...
    hanzo.trace({ id: parentTraceId, name: traceName });
