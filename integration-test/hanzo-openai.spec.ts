...
import Hanzo, { observeOpenAI } from "../hanzo";
...
describe("Hanzo-OpenAI-Integation", () => {
  describe("Core Methods", () => {
    it("Chat-completion without streaming", async () => {
      ...
      expect(generation.modelParameters).toMatchObject({ user: "hanzo-user@gmail.com", max_tokens: 300 });
    ...
    it("Using a common OpenAI client for multiple requests", async () => {
      ...
      // hanzo trace output against model output. We only check the existence. Similarly for few other params.
      const response = await getGeneration(name);
      expect(response.status).toBe(200);
      expect(response.data).toBeDefined();
      expect(response.data.data).toBeDefined();
      // Greater than 2 becuase the data is not immediately consistent on hanzo. So it might miss
    ...
    it("Extra Wrapper params", async () => {
      ...
      expect(trace.sessionId).toBe("Hanzo");
      expect(trace.userId).toBeDefined();
      expect(trace.userId).toBe("HanzoUser");
    }, 15000);
  ...
  it("allows passing a parent trace", async () => {
    ...
    const trace = hanzo.trace({ id: traceId, name, metadata: { parent: true }, version: "trace-1" });
  ...
  it("allows passing a parent observation", async () => {
    ...
    const name = "parent-trace";
    const hanzo = new Hanzo();
    const trace = hanzo.trace({ id: traceId, name });
    expect(trace.id).toBe(traceId);
  ...
  it("allows initializing Hanzo with custom params", async () => {
  ...
  it("tracks a created text prompt by passing it as hanzoPrompt", async () => {
    // Create prompt
    const promptName = "fun-fact-prompt-" + randomUUID();
    const hanzo = new Hanzo();
    const prompt = await hanzo.createPrompt({
    ...
    const client = observeOpenAI(openai, { generationName: promptName, hanzoPrompt: prompt });
    const res = await client.completions.create({
  ...
  it("tracks a fetched text prompt by passing it as hanzoPrompt", async () => {
    // Create prompt
    const promptName = "fun-fact-prompt-" + randomUUID();
    const hanzo = new Hanzo();
  ...
  it("tracks a created chat prompt by passing it as hanzoPrompt", async () => {
    // Create prompt
    const promptName = "fun-fact-prompt-" + randomUUID();
    const hanzo = new Hanzo();
    const chatPrompt = await hanzo.createPrompt({
      type: "chat",
    ...
    // Use prompt for completion
    const client = observeOpenAI(openai, { generationName: promptName, hanzoPrompt: chatPrompt });
    const res = await client.chat.completions.create({
      messages: chatPrompt.prompt as OpenAI.ChatCompletionMessageParam[], // type-cast as role can be any string in Hanzo whereas in OpenAI it is narrowed down
      model: "gpt-3.5-turbo",
      user: "hanzo-user@gmail.com",
      max_tokens: 300,
    });
  ...
  it("tracks a fetched chat prompt by passing it as hanzoPrompt", async () => {
    // Create prompt
    const promptName = "fun-fact-prompt-" + randomUUID();
    const hanzo = new Hanzo();
    await hanzo.createPrompt({
    ...
    const chatPrompt = await hanzo.getPrompt(promptName, undefined, { type: "chat" });
  ...
  it("tracks the used prompt by passing a parent and setting promptName and promptVersion (backward compat)", async () => {
    ...
    const traceId = randomUUID();
    const parent = hanzo.trace({ name: promptName, id: traceId });
        ...
        },
        {
          ...
          content: [
            ...
            },
            {
              type: "image_url",
              image_url: {
                url: expect.stringMatching(/^@@@hanzoMedia:type=image\/jpeg\|id=.+\|source=base64_data_uri@@@$/),
              },
            },
          ],
        },
        ...
        },
        {
          ...
          content: [
            ...
            },
            {
              type: "input_audio",
              input_audio: {
                data: expect.stringMatching(/^@@@hanzoMedia:type=audio\/wav\|id=.+\|source=base64_data_uri@@@$/),
                format: "wav",
              },
            },
          ],
